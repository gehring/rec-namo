...
class RandomProjector(Projector):
    def __init__(self, projector, n_dim):
        super(RandomProjector, self).__init__()
        self.phi = projector
#         self.random_proj = sklearn.random_projection.SparseRandomProjection(n_components= n_dim)
        self.random_proj = sklearn.random_projection.sparse_random_matrix(n_components=n_dim, 
                                                                          n_features=self.phi.size).T
                                                                          
#         self.random_proj = sklearn.random_projection.gaussian_random_matrix(n_components=n_dim, 
#                                                                           n_features=self.phi.size).T
        self.n_dim = n_dim
        
    def __call__(self, state):
        return (self.phi(state).dot(self.random_proj))
